...
<head>
    ...
    <title>C 語言 第四章 詳細追蹤練習 (2/3) - 詳解版</title>
    ...
    <style>
        ...
        .interactive-panel {
            flex: 1 1 45%; /* Initial width, allows growing/shrinking */
            min-width: 400px;
            height: 100vh;
            padding: 20px;
            box-sizing: border-box;
        ...
        .explanation-content-area table {
            width: 100%;
            /* border-collapse: collapse; */ /* Removed for better visibility */
            border-spacing: 0; /* Use instead of collapse */
            margin-top: 15px;
            margin-bottom: 15px;
            font-size: 0.9em;
            background-color: var(--code-bg);
            border: 1px solid var(--border-color); /* Add outer border to table */
            border-radius: 4px;
            overflow: hidden; /* Ensures inner elements conform to border-radius */
        }
        .explanation-content-area th, .explanation-content-area td {
            border-bottom: 1px solid var(--border-color); /* Horizontal lines */
            border-right: 1px solid var(--border-color); /* Vertical lines */
            padding: 8px 10px;
            text-align: left;
            vertical-align: top;
        }
        /* Remove border from the last cell in a row */
        .explanation-content-area tr td:last-child,
        .explanation-content-area tr th:last-child {
            border-right: none;
        }
        /* Remove border from the last row */
        .explanation-content-area tr:last-child td {
            border-bottom: none;
        }
        .explanation-content-area th {
            background-color: var(--primary-color);
            color: white;
            font-weight: bold;
        }
        .explanation-content-area td code {
             background-color: rgba(255,255,255,0.1);
             padding: 1px 4px;
        }
        .run-example-btn {
            display: none; /* Hide run example button */
        }
        ...
        #explanation-content-area ul, #explanation-content-area ol {
            padding-left: 20px;
            margin-bottom: 10px;
        }
        #explanation-content-area ul li::marker {
            color: var(--primary-color);
        }
        .quiz-section {
            margin-top: 30px;
        ...
        .quiz-options .option.incorrect {
            ...
            background-color: rgba(106, 95, 95, 0.15);
        }
...
<body>
    <div class="container">
        <main class="tutorial-content" id="left-panel">
            <h1>C 語言練習：第四章 詳細變數追蹤 (2/3)</h1>
            <p>本頁面提供 C 語言第四章的互動練習題。請仔細分析題目，並選擇您認為正確的答案。點擊選項後，將會在<b>右側互動區塊</b>顯示該題的詳細解說。</p>
            <div class="quiz-section">
                <h2>第四章 互動練習題 - 詳細追蹤版 (題目 16-30)</h2>
                <p>請挑戰下面的題目，檢驗您的學習成果！點擊選項後，詳解將顯示於右側面板。</p>
                <!-- QUIZ CARDS START -->
...
*/</code></pre>
                    ...
                    <div class="explanation">
                        ...
                        <table style="border:3pxrgb(135, 133, 133) solid;" cellpadding="4" border='1'>
                            <thead>
...
*/</code></pre>
                    ...
                    <div class="explanation">
                        <h4>✓ 解題思路與變數追蹤</h4>
                        <p>變數 <code>k</code> 初始值為 2。在每次迴圈中，<code>k</code> 的值會變成其自身的平方 (<code>k = k * k</code>)。迴圈持續的條件是 <code>k &lt;= 65535</code>。</p>
                        <table style="border:3pxrgb(135, 133, 133) solid;" cellpadding="4" border='1'>
                            <thead>
...
int main(){
  ...
  return 0;
                    ...
                    <div class="explanation">
                        <h4>✓ 解題思路與變數追蹤</h4>
                        <p>此程式使用 <code>for</code> 迴圈。變數 <code>y1</code> 從 0 開始，每次迴圈體執行後增加 2。變數 <code>y3</code> 初始為 1，並在迴圈體內累加 <code>y1</code> 的值。迴圈的條件是 <code>y1 &lt;= y2</code> (即 <code>y1 &lt;= 13</code>)。</p>
                        <table style="border:3pxrgb(135, 133, 133) solid;" cellpadding="4" border='1'>
                            <thead>
...
int main(){
  ...
  return 0;
        ...
        <aside class="interactive-panel" id="right-panel">
            <div class="interactive-panel-inner">
                <div id="explanation-container">
                    <h3>詳解</h3>
                    <div id="explanation-content-area">
                        <p>請點擊左側任一題目的選項，此處將會顯示該題的詳細解說。</p>
                    </div>
                </div>
            </div>
        </aside>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function handleMouseMove(e) {
            ...
            if (containerRect.width - leftWidth < rightMinWidth) {
                leftWidth = containerRect.width - rightMinWidth;
            }
        ...
        quizOptions.forEach(optionsContainer => {
            ...
            options.forEach(option => {
                option.addEventListener('click', function () {
                    ...
                    // Mark all options in the same card as answered
                    const parentCard = this.closest('.quiz-card');
                    const siblingOptions = parentCard.querySelectorAll('.option');
                    let cardIsAlreadyAnswered = false;
                    siblingOptions.forEach(opt => {
                        if(opt.classList.contains('answered')) {
                            cardIsAlreadyAnswered = true;
                        }
                    });
                    if(cardIsAlreadyAnswered) return;

                    siblingOptions.forEach(opt => opt.classList.add('answered'));


                    const selectedOption = option.getAttribute('data-option');
                    const explanationSource = optionsContainer.parentElement.querySelector('.explanation');
                    ...
                    if (explanationSource) {
                        explanationContentArea.innerHTML = explanationSource.innerHTML;
                        if (typeof MathJax !== 'undefined' && MathJax.typeset) {
                            MathJax.typeset([explanationContentArea]);
                        }
                    } else {
                        explanationContentArea.innerHTML = '<p>此題沒有提供詳解。</p>';
                    }
                });
            });
        });
