...
<ul>
    ...
    <li><strong>(D) 一a一long一name：</strong>這個變數名以底線開頭 (題目中的長橫線應理解為底線)，後面跟著字母、數字和底線的組合 (假設題目中的 "一" 是底線的另一種表示或筆誤)。如果將所有 "一" 視為底線 <code>_</code>，則 <code>_a_long_name</code> 是一個合法的 C++ 變數名。它以底線開頭，後續字元為字母和底線。</li>
</ul>
<p><strong>3. 結論：</strong></p>
...
<ul>
    ...
    <li><strong>不需要分號 <code>;</code></strong> 在指令的末尾。如果加了分號，分號本身也會成為 替換文本 的一部分，這通常不是期望的行為 (除非刻意為之，例如在定義多行巨集時)。</li>
</ul>
...
<ul>
    ...
    <li><strong><code>char</code>：</strong>字元型態。用於儲存單個字元 (如 'a', '
, '7')。它實際上是小整數型態，通常佔用 1 byte，儲存的是字元的 ASCII 值或其他字元編碼值。</li>
</ul>
...
// 處理 AJAX 請求
if (isset($_POST['action']) && $_POST['action'] == 'get_explanation') {
    $question_index = intval($_POST['question_index']);
    if (isset($questions[$question_index])) {
        $explanation = $questions[$question_index]['explanation_html'];
        $user_answer = $_POST['user_answer'];
        $correct_answer = $questions[$question_index]['correct_answer'];
        
        $result_header = "";
        if ($user_answer == $correct_answer) {
            $result_header = "<h3 class='correct'>答案正確！</h3>";
        } else {
            $result_header = "<h3 class='incorrect'>答案錯誤！正確答案是：{$correct_answer}</h3>";
        }

        echo $result_header . $explanation;
    } else {
        echo "<p>找不到詳解。</p>";
    }
    exit; // 結束腳本執行
...
<div class="container">
    <div class="left-pane" id="left-pane">
        <h1>C/C++ 綜合測驗</h1>
        <?php foreach ($questions as $index => $exercise): ?>
            <div id="q<?php echo htmlspecialchars($exercise['id_suffix']); ?>" class="quiz-card">
                ...
                <div class="quiz-options" data-question-index="<?php echo $index; ?>">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.addEventListener('mousemove', function(e) {
            ...
            const leftFlexBasis = leftWidth;
            const rightFlexBasis = containerRect.width - leftWidth - handle.offsetWidth;

            leftPane.style.flex = `0 1 ${leftFlexBasis}px`;
            rightPane.style.flex = `0 1 ${rightFlexBasis}px`;
        });
        ...
        options.forEach(option => {
            option.addEventListener('click', function() {
                ...
                const questionIndex = parentOptions.getAttribute('data-question-index');
                const userAnswer = this.getAttribute('data-option');

                // 使用 Fetch API 發送請求
                const formData = new FormData();
                formData.append('action', 'get_explanation');
                ...
                .then(data => {
                    explanationArea.innerHTML = data;
                })
                .catch(error => {
                    console.error('Error:', error);
                    explanationArea.innerHTML = '<p>載入詳解時發生錯誤。</p>';
                });
            });
        });
    });
