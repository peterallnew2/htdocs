...
<body>
    ...
    <div class="container">
            ...
            <div class="quiz-section" id="quiz-section-dynamic">
                ...
                <?php foreach ($current_exercises as $index => $exercise): ?>
                    <div id="q<?php echo htmlspecialchars($exercise['id_suffix']); ?>" class="quiz-card">
                       ...
                       <script>
                       // JavaScript to add onclick event to options
                       document.addEventListener('DOMContentLoaded', function() {
                           const quizCard = document.getElementById('q<?php echo htmlspecialchars($exercise['id_suffix']); ?>');
                           const options = quizCard.querySelectorAll('.option');

                           options.forEach(option => {
                               option.onclick = function() {
                                   showExplanation('q<?php echo htmlspecialchars($exercise['id_suffix']); ?>');
                               };
                           });
                       });
                       </script>
        ...
        <aside class="interactive-panel">
            <div class="interactive-panel-inner">
                <div id="interactive-panel-explanation"></div>
    ...
    <script>
        var explanations = {
            <?php
            foreach ($current_exercises as $index => $exercise) {
                $explanation_id = 'q' . htmlspecialchars($exercise['id_suffix']);
                $explanation_html = str_replace(["\r\n", "\n", "\r"], "\\n", addslashes($exercise['explanation_html']));
                echo "'" . $explanation_id . "': \"" . $explanation_html . "\",\n";
            }
            ?>
        };

        function showExplanation(questionId) {
            const explanationHTML = explanations[questionId];
            if (explanationHTML) {
                document.getElementById('interactive-panel-explanation').innerHTML = explanationHTML;
            } else {
                document.getElementById('interactive-panel-explanation').innerHTML = "<p>No explanation available.</p>";
            }
        }

        // function showOption(option) {
        //   document.getElementById('interactive-panel-explanation').innerHTML = option;
        // }
    </script>
</body>
</html>
